...
class PostgresProjectRepository(ProjectRepository):
    ...
    async def get_by_id(self, id: str) -> Optional[Project]:
        result = await self._session.execute(text(f"SELECT {SEL} FROM pamp_project WHERE id::text = :id OR project_id = :id"), {"id": id})
        row = result.fetchone()
        return self._to_entity(row) if row else None

    async def list(self, page: int = 1, page_size: int = 20) -> tuple[list[Project], int]:
        count_result = await self._session.execute(text("SELECT COUNT(*) FROM pamp_project"))
        total = count_result.scalar()
        offset = (page - 1) * page_size
        result = await self._session.execute(text(f"SELECT {SEL} FROM pamp_project ORDER BY created_at DESC LIMIT :limit OFFSET :offset"), {"limit": page_size, "offset": offset})
        items = [self._to_entity(row) for row in result.fetchall()]
        return items, total

    async def list_by_owner(self, owner_id: str) -> list[Project]:
        result = await self._session.execute(text(f"SELECT {SEL} FROM pamp_project WHERE owner_id = :owner_id ORDER BY created_at DESC"), {"owner_id": owner_id})
    ...
    async def delete(self, id: str) -> bool:
        result = await self._session.execute(text("DELETE FROM pamp_project WHERE id::text = :id OR project_id = :id"), {"id": id})
...
class PostgresTeamMemberRepository(TeamMemberRepository):
    ...
    async def get_by_id(self, id: str) -> Optional[TeamMember]:
        result = await self._session.execute(text("SELECT id, project_id, user_itcode, role FROM pamp_team_member WHERE id = :id"), {"id": id})
        row = result.fetchone()
        return self._to_entity(row) if row else None

    async def list(self, page: int = 1, page_size: int = 20) -> tuple[list[TeamMember], int]:
        offset = (page - 1) * page_size
        count_result = await self._session.execute(text("SELECT COUNT(*) FROM pamp_team_member"))
        total = count_result.scalar()
        result = await self._session.execute(text("SELECT id, project_id, user_itcode, role FROM pamp_team_member ORDER BY user_itcode LIMIT :limit OFFSET :offset"), {"limit": page_size, "offset": offset})
        return [self._to_entity(row) for row in result.fetchall()], total

    async def list_by_project(self, project_id: str) -> list[TeamMember]:
        result = await self._session.execute(text("SELECT id, project_id, user_itcode, role FROM pamp_team_member WHERE project_id = :project_id"), {"project_id": project_id})
        return [self._to_entity(row) for row in result.fetchall()]

    async def create(self, entity: TeamMember) -> TeamMember:
        await self._session.execute(text("INSERT INTO pamp_team_member (id, project_id, user_itcode, role) VALUES (:id, :project_id, :user_itcode, :role)"), {"id": str(entity.id), "project_id": entity.project_id, "user_itcode": entity.user_itcode, "role": entity.role})
        return entity

    async def update(self, entity: TeamMember) -> TeamMember:
        await self._session.execute(text("UPDATE pamp_team_member SET role=:role WHERE id=:id"), {"id": str(entity.id), "role": entity.role})
        return entity

    async def delete(self, id: str) -> bool:
        result = await self._session.execute(text("DELETE FROM pamp_team_member WHERE id = :id"), {"id": id})
        return result.rowcount > 0
