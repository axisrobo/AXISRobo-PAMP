...
class PostgresReviewRequestRepository(ReviewRequestRepository):
    ...
    async def get_by_id(self, id: str) -> Optional[ReviewRequest]:
        result = await self._session.execute(
            text("SELECT id, request_id, title, description, status, submitter, reviewer, submitted_at, reviewed_at, created_at FROM pamp_request WHERE id = :id"),
            {"id": id}
        )
        row = result.fetchone()
    ...
    async def list(self, page: int = 1, page_size: int = 20) -> tuple[list[ReviewRequest], int]:
        count_result = await self._session.execute(text("SELECT COUNT(*) FROM pamp_request"))
        total = count_result.scalar()
        offset = (page - 1) * page_size
        result = await self._session.execute(
            text("SELECT id, request_id, title, description, status, submitter, reviewer, submitted_at, reviewed_at, created_at FROM pamp_request ORDER BY created_at DESC LIMIT :limit OFFSET :offset"),
    ...
    async def list_by_submitter(self, submitter: str) -> list[ReviewRequest]:
        result = await self._session.execute(
            text("SELECT id, request_id, title, description, status, submitter, reviewer, submitted_at, reviewed_at, created_at FROM pamp_request WHERE submitter = :submitter ORDER BY created_at DESC"),
            {"submitter": submitter}
        )
    ...
    async def list_by_reviewer(self, reviewer: str) -> list[ReviewRequest]:
        result = await self._session.execute(
            text("SELECT id, request_id, title, description, status, submitter, reviewer, submitted_at, reviewed_at, created_at FROM pamp_request WHERE reviewer = :reviewer ORDER BY created_at DESC"),
            {"reviewer": reviewer}
        )
        return [self._to_entity(row) for row in result.fetchall()]
    ...
    async def list_by_status(self, status: str) -> list[ReviewRequest]:
        result = await self._session.execute(
            text("SELECT id, request_id, title, description, status, submitter, reviewer, submitted_at, reviewed_at, created_at FROM pamp_request WHERE status = :status ORDER BY created_at DESC"),
            {"status": status}
        )
        return [self._to_entity(row) for row in result.fetchall()]
    ...
    async def get_dashboard_stats(self) -> dict:
        count_result = await self._session.execute(text("SELECT COUNT(*) FROM pamp_request"))
        total = count_result.scalar()
        draft_result = await self._session.execute(text("SELECT COUNT(*) FROM pamp_request WHERE status = 'draft'"))
        return {"total": total, "draft": draft_result.scalar()}

    async def create(self, entity: ReviewRequest) -> ReviewRequest:
        await self._session.execute(
            text("INSERT INTO pamp_request (id, request_id, title, description, status, submitter, reviewer, submitted_at, reviewed_at, created_at) VALUES (:id, :request_id, :title, :description, :status, :submitter, :reviewer, :submitted_at, :reviewed_at, :created_at)"),
    ...
    async def update(self, entity: ReviewRequest) -> ReviewRequest:
        await self._session.execute(
            text("UPDATE pamp_request SET title=:title, description=:description, status=:status, reviewer=:reviewer, reviewed_at=:reviewed_at WHERE id=:id"),
    ...
    async def delete(self, id: str) -> bool:
        result = await self._session.execute(text("DELETE FROM pamp_request WHERE id = :id"), {"id": id})
...
class PostgresMeetingRepository(MeetingRepository):
    ...
    async def list_by_request(self, request_id: str) -> list[Meeting]:
        result = await self._session.execute(
            text("SELECT id, request_id, title, meeting_date, location, attendees, minutes, status FROM pamp_meetings WHERE request_id = :request_id ORDER BY meeting_date DESC"),
...
class PostgresActionRepository(ActionRepository):
    ...
    async def list_by_request(self, request_id: str) -> list[Action]:
        result = await self._session.execute(
            text("SELECT id, request_id, meeting_id, description, assignee, due_date, status FROM pamp_actions WHERE request_id = :request_id ORDER BY due_date"),
            {"request_id": request_id}
        )
        return [self._to_entity(row) for row in result.fetchall()]
    ...
    async def list_by_assignee(self, assignee: str) -> list[Action]:
        result = await self._session.execute(
            text("SELECT id, request_id, meeting_id, description, assignee, due_date, status FROM pamp_actions WHERE assignee = :assignee ORDER BY due_date"),
            {"assignee": assignee}
        )
        return [self._to_entity(row) for row in result.fetchall()]
...
class PostgresAttachmentRepository(AttachmentRepository):
    ...
    async def list_by_request(self, request_id: str) -> list[Attachment]:
        result = await self._session.execute(
            text("SELECT id, request_id, filename, content_type, s3_key, size_bytes, uploaded_at FROM pamp_request_attachment WHERE request_id = :request_id ORDER BY uploaded_at DESC"),
            {"request_id": request_id}
        )
        return [self._to_entity(row) for row in result.fetchall()]
