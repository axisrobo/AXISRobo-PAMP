    ...
    └─ 调用 get_auth_provider()
           ...
           └─ resolve_scoped_roles(user, db)
                ...
                └─ 查询数据库中�?EA_Reviewer/App_Owner 角色分配
                     ...
                     ├─ 是否�?pamp_bigea_team_members 中？ �?EA_Reviewer
...
# 使用 pamp.fiscal_year_sequences 表实现原子自�?
...
##### 数据模型（DB: `pamp.pamp_request_attachment`�?
...
##### 关联表（DB: `pamp.pamp_arch_ai_check`�?
...
##### 数据模型（DB: `pamp.pamp_arch_ai_check_app`�?
...
- `pamp.project_app` �?项目维度的应用信�?
- `pamp.cmdb_application` �?�?CMDB 同步的应用基础信息

- `pamp.biz_cap_map` �?业务能力映射（BCM�?
- `pamp.bcpf_master_data` �?BizCapability 主数�?
...
##### 数据模型（DB: `pamp.tech_key_stack_item`�?
...
##### 数据模型（DB: `pamp.pamp_bigea_team_members`�?
...
| GET `/data-classification` | `pamp.data_classification` | 数据分类（树形，有层级） |

| GET `/data-centers` | `pamp.data_center` | 数据中心列表 |

| GET `/legal-entities` | `pamp.legal_entity` | 法律实体列表 |

| POST/PUT/DELETE `/data-centers/{id}` | `pamp.data_center` | 数据中心 CRUD（EA_Admin�?|

| POST/PUT/DELETE `/legal-entities/{id}` | `pamp.legal_entity` | 法律实体 CRUD（EA_Admin�?|
...
文件上传�?S3
    ...
    └─�?process_attachment_ai_check(attachment_id, db, user)
              ...
              �?
              └─ 保存识别到的应用�?pamp.pamp_arch_ai_check_app
...
| CMDB 应用同步 | 每日 02:00 (Asia/Shanghai) | �?CMDB API 全量拉取应用数据，upsert �?`pamp.cmdb_application` |
...
所有表位于 `pamp` schema（PostgreSQL），连接时通过 `SET search_path TO pamp` 自动设置�?


### 5.2 核心表结�?


#### `pamp.pamp_request` �?EA 审查请求
...
#### `pamp.pamp_request_process_log` �?请求流程日志
...
request_id  VARCHAR          -- 关联 pamp_request.request_id
...
#### `pamp.pamp_request_attachment` �?请求附件
...
#### `pamp.pamp_arch_ai_check` �?AI 架构检查结�?
...
attachment_uuid  UUID             -- 关联 pamp_request_attachment.id
...
#### `pamp.pamp_arch_ai_check_app` �?AI 识别的应�?
...
ai_check_id      UUID             -- 关联 pamp_arch_ai_check.id
...
#### `pamp.cmdb_application` �?CMDB 应用（定时同步）
...
#### `pamp.pamp_bigea_team_members` �?EA 团队成员
...
#### `pamp.tech_key_stack_item` �?技术栈条目
...
#### `pamp.tech_stack_operate_log` �?技术栈变更日志（Migration 001�?
...
#### `pamp.fiscal_year_sequences` �?财年序列�?
...
#### `pamp.data_classification` / `pamp.data_center` / `pamp.legal_entity` �?主数据参考表
...
CMDB API (每日 02:00)
    ...
    └─ cmdb_sync.py �?pamp.cmdb_application



用户手动维护

    └─ pamp.project_app (项目应用信息)
...
BCM 创建（App Owner�?
    └─ POST /applications/bcm
         ...
         └─ pamp.biz_cap_map (app_id + bcpf_master_id)
