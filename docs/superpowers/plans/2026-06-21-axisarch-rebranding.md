...
**Goal:** Transform the legacy enterprise architecture application into AxisArch — zero legacy branding, complete documentation suite, harness aligned with the actual codebase.
...
All vendor-specific defaults become empty strings or generic placeholders:
...
class Settings(BaseSettings):
    ...
    OBSERVABILITY_APPLICATION_NAME: str = "AxisArch Review Assistant"
    OBSERVABILITY_SERVICE_NAME: str = "AxisArch Review Assistant"
    OBSERVABILITY_TOKEN: str = ""
    OBSERVABILITY_COLLECTOR_URL: str = ""
    OBSERVABILITY_GATEWAY_TOKEN_URL: str = ""
    OBSERVABILITY_GATEWAY_ACCOUNT: str = ""
    OBSERVABILITY_GATEWAY_KEY: str = ""
    OBSERVABILITY_GATEWAY_SECRET: str = ""
    ...
    MESSAGE_TOKEN_URL: str = ""
    MESSAGE_APP_CODE: str = ""
    MESSAGE_SDK_KEY: str = ""
...
Rewrite to generic:
```
DATABASE_URL=postgresql+asyncpg://<user>:<password>@localhost:5432/axisarch
DB_SCHEMA=axisarch
PORT=4000
HOST=0.0.0.0
AUTH_DISABLED=true
...
- Line ~103: replace the vendor-specific external identity label with `SSO Provider (external users)`
- Line ~148: replace the organization-specific authentication sentence with `Internal users authenticate via enterprise IDP, external users via SSO Provider`
...
- Modify: `frontend/public/legacy-brand.png` → `frontend/public/axisarch.png`
...
```powershell
Rename-Item frontend/public/legacy-brand.png frontend/public/axisarch.png
```
...
```bash
git add frontend/src/components/ui/LoginGate.tsx frontend/public/axisarch.png frontend/public/legacy-brand.png frontend/src/app/\(data_management\)/help/page.tsx frontend/src/modules/technology_stack_management/
...
- Replace organization-specific solution-architect certification names with `Certified Solution Architect`
- Replace organization-specific technical-architect certification names with `Certified Technical Architect`

- [ ] **Step 3: Update ea-request-flow spec**

In `openspec/specs/ea-request-flow/spec.md`:
- Replace the legacy review URL with `<configured-review-url>`
- Replace the legacy notification mailbox with `<configured-notification-email>`
...
- Replace the vendor-specific CMDB name with `external CMDB API`
...
- Replace vendor-specific identity provider names with `SSO Provider or Enterprise IDP`
...
- Replace organization-qualified subsidiary wording with `Subsidiary / JV / holding data`
- Replace organization-qualified solution wording with `Have existing solution in organization`
- Replace organization-qualified portfolio wording with `Solution uses existing technologies within organizational portfolio`

- [ ] **Step 4: Update questionnaire_config.py**

In `backend/app/avdm/questionnaire_config.py`:
- line 189: `"Subsidiary / JV / holding data",`
- line 192: `{"label": "Example Organization", "value": "example_org"},`

- [ ] **Step 5: Update master_data_seeds.py**

In `backend/app/avdm/master_data_seeds.py`:
- Replace vendor-specific identity references with generic SSO
...
```python
# line 18: replace
{"id": 1, "company_code": "EXAMPLE01", "company_name": "Example Organization"},
# line 82: replace
...
- Replace the legacy personal test address with `testuser@example.com`
...
- Remove vendor-specific API gateway references → generic
- Replace example URLs with placeholders
- Replace the organization-specific multi-data-center title with a generic example
...
6. **Email service coupled**: Hardcoded to a vendor-specific messaging API — not abstracted
...
- Replace all legacy product references with AxisArch
